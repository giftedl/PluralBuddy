...
function Card({
  ...
  size = "default",
  ...props
}: React.ComponentProps<"div"> & { size?: "default" | "sm", noFlexCol?: boolean }) {
  return (
    <div
      data-slot="card"
      data-size={size}
      className={cn("ring-foreground/10 border bg-card text-card-foreground gap-4 overflow-hidden rounded-none py-4 text-xs/relaxed has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:gap-2 data-[size=sm]:py-3 data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-none *:[img:last-child]:rounded-none group/card flex", noFlexCol ? "" : "flex-col", className)}
      {...props}
    />
  )
}

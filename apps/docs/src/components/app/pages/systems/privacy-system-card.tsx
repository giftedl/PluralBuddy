...
export function PrivacySystemSettingsCard({ data }: { data: PSystem }) {
	const [values, setValues] = useState<{ name: string; value: string }[]>(
		flags.map((v) => ({
			name: v.name,
			value: data.public & v.value ? "public" : "private",
		})),
	);
		...
		<Card className="mb-3">
			<CardContent>
				...
				>
					...
					<CardDescription>
						Configure how others can see and use your system. By default, all
						system data is private.{" "}
						<strong>
							This doesn't impact your alter's individual privacy settings.
						</strong>
					</CardDescription>
				</motion.div>
				<motion.div
				...
				>
					{flags.map((flag, i) => (
						<motion.div
							initial={{ opacity: 0, y: -10 }}
							animate={{ opacity: 100, y: 0 }}
							transition={{ type: "tween", delay: 0.4 + i * 0.1 }}
							key={flag.name}
						>
							<Field className="flex! justify-between flex-row! items-center">
								<div className="block">

  ...
  return (
    <NavProvider transparentMode={nav?.transparentMode}>
      <main
        id="nd-home-layout"
        {...rest}
        className={cn('flex flex-1 flex-col xl:pt-14', rest.className)}
      >
        {nav.enabled !== false &&
          (nav.component ?? (
            <Header
...
export function Header({
  ...
  return (
    <Navbar>
      <Link
        href={nav.url ?? '/'}
        className="inline-flex items-center gap-2.5 font-semibold hover:text-accent-foreground transition-all"
      >
        {nav.title}
      </Link>
      {nav.children}

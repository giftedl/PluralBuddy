...
export const ensureGuildPermissions = createMiddleware<void>(async (middle) => {
	...
	const member = await ctx.client.members
		.fetch(ctx.guildId, ctx.author.id)
		.catch(() => null);
	const pluralGuild = PGuildObject.parse(
		(await ctx.client.cache.pguild.get(ctx.guildId ?? ""))?.g ??
			(await getGuildFromId(ctx.guildId ?? "")),
	);
	...
	const managerRolesIntersect =
		memberRoles.filter((c) => pluralGuild.managerRoles.includes(c)).length >= 1;

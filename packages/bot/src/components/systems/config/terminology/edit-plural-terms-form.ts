...
export default class SetPronounsButton extends ModalCommand {
	override filter(context: ModalContext) {
		return InteractionIdentifier.Systems.Configuration.TerminologyTab.EditPluralTermsForm.startsWith(
			context.customId,
		);
	}
	...
	override async run(ctx: ModalContext) {
		...
		const [alter, tag, systemArgument, proxyTag] = [
			...
			{ userId: ctx.author.id },
			{
				$set: {
					terminology: {
						...
						alters_plural: (alter as string) ?? "",
						tags_plural: (tag as string) ?? "",
						system_plural: (systemArgument as string) ?? "",
						proxy_tags_plural: (proxyTag as string) ?? "",
					},
				},
			},
		);

...
export default class SetPronounsButton extends ModalCommand {
	override filter(context: ModalContext) {
		return InteractionIdentifier.Systems.Configuration.FormSelection.SystemDescriptionForm.startsWith(
			context.customId,
		);
	}
	...
	override async run(ctx: ModalContext) {
		...
		const newSystemDescription = ctx.interaction.getInputValue(
			InteractionIdentifier.Systems.Configuration.FormSelection.SystemDescriptionType.create(),
			true,
		);

...
export default class SetNameForm extends ModalCommand {
  ...
  override filter(context: ModalContext) {
    return InteractionIdentifier.Systems.Configuration.FormSelection.NameForm.startsWith(context.customId);
  }

  async run(ctx: ModalContext) {
    const newSystemName = ctx.interaction.getInputValue(InteractionIdentifier.Systems.Configuration.FormSelection.NameType.create(), true)

...
import { ActionRow, Attachment, AttachmentBuilder, Button, ComponentCommand, File, type ComponentContext } from 'seyfert';
...
export default class DeleteSystemButton extends ComponentCommand {
    ...
    async run(ctx: ComponentContext<typeof this.componentType>) {
        ...
        await ctx.author.write({
            content: ctx.userTranslations().SYSTEM_DELETION_DM.replace("%time%", `<t:${Math.floor(Date.now() / 1000)}>`),
            files: [
        ...
        return await ctx.editResponse({
            components: new AlertView(ctx.userTranslations()).successView("SYSTEM_DELETION_FINISHED"),
            flags: MessageFlags.Ephemeral + MessageFlags.IsComponentsV2
        })
      }
}
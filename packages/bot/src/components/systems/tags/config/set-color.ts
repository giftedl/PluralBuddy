...
export default class SetUsernameButton extends ComponentCommand {
	...
	override filter(context: ComponentContext<typeof this.componentType>) {
		return InteractionIdentifier.Systems.Configuration.Tags.SetColor.startsWith(
			context.customId,
		);
	}
	...
	override async run(ctx: ComponentContext<typeof this.componentType>) {
		const tagId =
			InteractionIdentifier.Systems.Configuration.Tags.SetColor.substring(
				ctx.customId,
			)[0];
		...
		const form = new Modal()
			.setCustomId(
				InteractionIdentifier.Systems.Configuration.FormSelection.Tags.TagColorForm.create(
					tag.tagId,
				),
			)

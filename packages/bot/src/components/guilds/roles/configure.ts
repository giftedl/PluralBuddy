...
export default class ConfigureRoleButton extends ComponentCommand {
	...
	override filter(context: ComponentContext<typeof this.componentType>) {
		return InteractionIdentifier.Guilds.RolesTab.Preference.startsWith(
			context.customId,
		);
	}

	override async run(ctx: ComponentContext<typeof this.componentType>) {
        const roleId = InteractionIdentifier.Guilds.RolesTab.Preference.substring(ctx.customId)[0] ?? "";

...
> **Pro tip:** Don't give all bots Administrator permissions! It increases your likelyhood of being raided if a bot is compromised.
...
However, for there are two permissions that you can use for proxying which requires **only 1** to be granted depending on the proxy mode you'd like in the environment.
-# If you'd like both modes to be enabled, you can grant both permissions.

> **Manage Nicknames:** Needed for Nickname proxy mode _(${userPerms?.has([PermissionFlagsBits.ManageNicknames]) ? "Granted" : "Denied"})_. Proxying user also needs the **Change Nickname** permission _(${sendingUserPerms?.has([PermissionFlagsBits.ChangeNickname]) ? "Granted" : "Denied"})_.
> **Manage Webhooks:** Needed for Webhook proxy mode _(${userPerms?.has([PermissionFlagsBits.ManageWebhooks]) ? "Granted" : "Denied"})_.


-# ALL permissions are needed in context of the channel. If any of the 5 above permissions aren't granted accordingly, in a channel, then the bot won't work in the way specified above.`),
					)
					.setColor("#FCCEE8"),
			],
		});

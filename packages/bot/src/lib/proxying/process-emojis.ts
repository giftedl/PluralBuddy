...
export async function processEmojis(
...
): Promise<{ emojis: ApplicationEmoji[]; newMessage: string }> {
	...
	for (const emoji of emojis) {
		...
		if (
			existingEmojis.find(
				(exEmoji: ApplicationEmoji) => emoji.name === exEmoji.name,
			) !== undefined
		) {
			emoji.name = `_${emoji.name}`.slice(0, 32);
		}

		const createdEmoji = await client.applications
			.createEmoji({
		...
		if (createdEmoji) uploadedEmojis.push(createdEmoji);
